...
- **Anything the user has to upload by hand must be one or two files, never hundreds.** The Odd One Out voice recorder wrote ~305 separate mp3s into a folder for the user to drag into GitHub's web uploader. They had to do it in batches, it bloated the repo with 300+ files, part of the upload silently never landed, and the manifest that tied it together got left behind so the game couldn't use *any* of them. The fix was to join every clip into one mp3 with a small index of where each word starts — same audio, one drag. The user is not a coder and uploads through the GitHub web UI: design any generated-asset pipeline around that from the start, not after they complain.
