...
- **A question is not an instruction to change something.** If the user asks about something ("why does X happen?", "can you do Y?", "what about Z?") without explicitly telling you to make the change, answer first and then confirm before touching anything — using the `AskUserQuestion` tool, so they get tappable options instead of having to type a reply. Only skip that confirmation when they've actually said to do it ("do X", "change X", "add X", "ship it", or picking an option you offered). Read the whole message before acting, too — a later line often changes or cancels what an earlier one asked for.
